...
; OUTPUTS:
;
;     1. One IDL save file with structures containing the observation,
;        processing and filaments description parameters  
;
;     2. Three ascii files for the same parameters as above:
;        observation,processing,description
;
;     3. One CDF file containing the subimages of filaments
;
;  
; MODIFICATION HISTORY:
;
;    NF dec 2004 last modif
;    NF 2010 small corrections for HELIO
;-

PRO EFR_FIL2ASCII_VM

;----------------------------------------------------------
; display issues
  DEVICE,DECOMPOSED=0,RETAIN=2
  DEVICE,GET_SCREEN_SIZE=scr_size
  IF scr_size[1] LT 1000 THEN display_size = 512 ELSE display_size = 1024
...
;-----------------------------------------------------------
  ...
  filters= ['*.fits', '*.fit', '*.fts', '*.fits.gz', '*.fit.gz']
  help,filters
  filenames = DIALOG_PICKFILE(PATH=ppath, /multiple_files, /FIX_FILTER, FILTER=filters, RESOURCE_NAME='Select one or more processed FITS file')
...
;     val07 = FXPAR(head,'SOLAR_R'      ,COUNT=count07)  
    ...
    ;-------------------------------------------------------- 
     ...
     IF FIX(strpro.CDELT1) EQ 0 OR FIX(strpro.CDELT2) EQ 0 OR $
        ...
        PRINT,'no cdelt or carrington rot. info in processed fits header!!'
        PRINT,'-> using SolarSoft GET_SUN function'
    ...
    ;--------------------------------------------------------
      ...
      arrf = EFR_FILAMENT_VM(obst,DOUBLE(2.*strpro.R_SUN),INPUT=TEMPORARY(arr),CORRECTED=corrected,/lin,/fla,/dus,DISPLAY=display_size,MAIND=maind)
    ...
    ;--------------------------------------------------------
    ; Remove small isolated faint regions
    ;--------------------------------------------------------
     im = EFR_REMOVE_SFIR2(im,corrected)
     TVSCL,CONGRID(corrected-im*0.5*strpro.QSUNINT,display_size/2.,display_size/2.),display_size/2.,display_size/2.
    ...
    strfil = EFR_FIL_DESCRIBE_VM(WHERE(im),WHERE(arrf),strfil,LONG(strpro.NAXIS1), $
    ...
    ;-------------------------------------------------------  
     ...
     resfile2 = splitf2[0] + obst +'_fi_init.txt'
     resfile3 = splitf2[0] + obst +'_fi_norm.txt'
     resfile4 = splitf2[0] + obst +'_fi_feat.txt'
     resfile7 = splitf2[0] + obst +'_fi.jpg'
    
    ;----------------------------------------------------------
    ; Save the 3 structures in one IDL file
    ;----------------------------------------------------------

;partie commentee par NF avril/2010 pour traitement
;de masse sans sauvegarde de structure IDL
...
;    SAVE,strobs,strpro,strfil,FILENAME=resfile1 
    ...
    ;######## FILE *fi_norm.txt ########
     ...
     PRINTF,un,STRTRIM(strpro.IND,2)+';'+$
	   ...
	   STRTRIM(strpro.NAXIS1,2)+';'+$;NF 2010 AJOUT DES 5 PARAMETRES SUIVANTS CAR L'IMAGE PROCESSEE N'EST PAS OBLIGATOIREMENT DU 1024*1024!
    ...
    ;--------------------------------------------------------
      ...
      IF strpro.NAXIS1 EQ 2048 THEN BEGIN
          res = ascii2fil(FILE=resfile4,/ske,/IM_2K,/NEW_NORM)
      ENDIF ELSE BEGIN 
          res = ascii2fil(FILE=resfile4,/ske,/NEW_NORM)
      ENDELSE
      ratio = 0.8*strpro.NAXIS1/display_size
      TVSCL,CONGRID(DILATE(res,replicate(1,3*ratio,3*ratio)),display_size/2.,display_size/2.),0,display_size/2.
      
      ;wait,0.5
      WRITE_JPEG,resfile7,TVRD()
      ;ERASE
      ;WHILE !d.window GE 35 DO BEGIN
      ;IF !d.window NE 9 THEN WDELETE,!d.window
      ;ENDWHILE
...
;    CDF_CLOSE,id
  ...
  ENDIF ; ( if there is something to describe )
