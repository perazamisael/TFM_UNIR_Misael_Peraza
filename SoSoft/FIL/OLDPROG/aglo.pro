...
;binary image: region 1, bckgd 0
  ...
  ;suppress round regions with no neighborhood
 
  FOR ii=0,nb_reg-1 DO BEGIN

    IF hist_labl(ii) LT 150 THEN BEGIN
      ...
      closepix = WHERE(dist GT (MAX(axes)/2.+1) AND dist LE 30,ccc)

      IF ccc EQ 0 AND MAX(axes)*1./MIN(axes) LE 3. THEN BEGIN
         im_cp(region) = 0
      ENDIF
