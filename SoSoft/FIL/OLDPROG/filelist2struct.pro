...
;         (DIALOG_PICKFILE)
;       - The original FITS observations headers
;
; OUTPUTS:
;
...
; Define path to store the ascii and cdf files
; Define local path 
;----------------------------------------------------------

  path  = '/data2/fuller/FITS/Ha/2002/PROCESSED/'
  opath = '/data2/fuller/FITS/Ha/2002/'    
  spath = '/home/fuller/poub/SAV/STRUCT/'
  lpath = '/Ha/2002/PROCESSED/'
...
;-----------------------------------------------------------
  ...
  filenames = DIALOG_PICKFILE(PATH=path, /multiple_files)
...
;     val07 = FXPAR(head,'SOLAR_R'      ,COUNT=count07)  
    ...
    ;-------------------------------------------------------  
     ...
     datef = STRMID(splitf2[0],2)
     resfile1 = spath + datef[0] + '_fil.sav'
     resfile2 = spath + datef[0] + '_fi_init.txt'
     resfile3 = spath + datef[0] + '_fi_norm.txt'
     resfile4 = spath + datef[0] + '_fi_feat.txt'
     resfile5 = spath + datef[0] + '_fi_ccode.txt'
     resfile6 = spath + datef[0] + '_fi_image.cdf'
    ...
    ;-------------------------------------------------------
     ...
     IF count15 GE 1 THEN date = STRTRIM(strobs.DATE_OBS,2) ELSE BEGIN
    ...
    ;-------------------------------------------------------- 
     ...
     obst = 0
     IF STRPOS(info1,'BBSO') NE -1 THEN obst = 3
     IF STRPOS(info1,'MEUDON') NE -1 OR STRPOS(info1,'PARIS') NE -1 AND $
        FIX(info2) EQ 1500 THEN obst = 2
     IF STRPOS(info1,'MEUDON') NE -1 OR STRPOS(info1,'PARIS') NE -1 AND $
        FIX(info2) LT 1500 THEN obst = 1
     IF obst EQ 0 THEN BEGIN
    ...
    CASE obst OF
      1: arrf = EFR_FILAMENT('meudon1',DOUBLE(2.*strpro.R_SUN), $
         INPUT=TEMPORARY(arr), CORRECTED=corrected,/lin,/fla,/dus,/dis)
      2: arrf = EFR_FILAMENT('meudon2',DOUBLE(2.*strpro.R_SUN), $
         INPUT=TEMPORARY(arr), CORRECTED=corrected,/lin,/fla,/dus,/dis)
      3: arrf = EFR_FILAMENT('bbso',DOUBLE(2.*strpro.R_SUN), $
         INPUT=TEMPORARY(arr), CORRECTED=corrected,/fla,/dis)
    ENDCASE
    ...
    strfil = EFR_FEAT_DESCRIBE(WHERE(im),strfil,FIX(strpro.NAXIS1), $
             FIX(strpro.NAXIS2),DOUBLE(strpro.CDELT1),DOUBLE(strpro.CDELT2), $
