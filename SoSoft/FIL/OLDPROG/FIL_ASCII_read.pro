...
CASE res[0] OF
    ...
    1: BEGIN
       ...
       PRINT, '# Min = ',STRTRIM(tab[0,0],2),' and Max = ',STRTRIM(tab[0,nbf-1],2)
       PRINT, '###################################################'
       READ, range1, range2, PROMPT='Enter range: '
       locs = WHERE(FIX(tab[0,*]) GE FIX(range1) AND FIX(tab[0,*]) LE FIX(range2))
       END
    ...
    2: BEGIN
       ...
       PRINT, '###################################################'
       READ, range1, range2, PROMPT='Enter range: '
       locs = WHERE(FLOAT(tab[38,*]) GE FLOAT(range1) AND FLOAT(tab[38,*]) LE FLOAT(range2))
       END 
    ...
    3: BEGIN
            ...
            IF jdstart2 NE jdend2 THEN BEGIN
               locs    = WHERE(tabtmp GE jdstart2 AND tabtmp LE jdend2)
            ENDIF ELSE BEGIN
               locs    = WHERE(tabtmp GE jdstart2 AND tabtmp LE jdstart2+1.)
            ENDELSE
    ...
    4: BEGIN
         ...
         PRINT, '################################################################'
         READ, range1, range2, PROMPT='Enter range: '
         locs = WHERE(FLOAT(tab[20,*]) GE FLOAT(range1) AND FLOAT(tab[20,*]) LE FLOAT(range2))
       END 
