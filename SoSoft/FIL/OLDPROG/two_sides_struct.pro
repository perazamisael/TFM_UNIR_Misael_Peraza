FUNCTION TWO_SIDES_STRUCT,input,structure
  ...
  result = REPLICATE(structure,num_REG-1)
  cmpt=0
  ...
  FOR i=1, num_REG-1 DO BEGIN
     ...
     result[i-1].fil_orientation = beta
     ...
     ;translate the object in the middle
     ...
     ;1:calculate an approximate line close to the border of interest (tab1)
     ;2:calculate an approximate line in the middle of the object (tab2)

     tab1 = INTARR(ymaxrot-yminrot+1,3)
     tab2 = INTARR(ymaxrot-yminrot+1,3) 
     wyy0 = WHERE(Y_rotemp EQ yminrot)
     wxx0 = X_rotemp[wyy0]
     xx_prev = wxx0[N_ELEMENTS(wxx0)/2]
     tab1[0,0:1] = [xx_prev,yminrot]  
     tab2[0,0:1] = [xx_prev,yminrot]
